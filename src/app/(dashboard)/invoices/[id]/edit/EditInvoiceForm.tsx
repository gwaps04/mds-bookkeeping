...
export default function EditInvoiceForm({ invoice, initialItems, customers, inventoryItems }: { invoice: any, initialItems: any[], customers: any[], inventoryItems: any[] }) {
  
  // Safely map initial DB items to the React state shape
  const safeInitialItems = initialItems.length > 0 
  ...
  // ============================================================================
  // THE CLIENT-SIDE ACTION WRAPPER (WITH DELTA-RECONCILIATION)
  // ============================================================================
  const clientAction = (formData: FormData) => {
    ...
    items.forEach(cartItem => {
      ...
      // DELTA-STATE MATH: 
      // Because this is an edit, the database already deducted the initial quantity.
      // We must mentally "add back" the initial quantity before subtracting the new cart quantity.
      const originalItemRecord = safeInitialItems.find(i => i.item_id === cartItem.item_id);
      const originalQty = originalItemRecord ? Number(originalItemRecord.quantity || 0) : 0;
      ...
      const projectedStock = (currentStock + originalQty) - orderQty;
    ...
    if (hasEmptyStock) {
      const confirm = window.confirm("🚨 CRITICAL WARNING: You are attempting to invoice items that are OUT OF STOCK.\n\nProceeding will push your inventory into the negative. Are you sure you want to process this edit?");
      if (!confirm) return; 
    } 
    else if (hasLowStock) {
      const confirm = window.confirm("⚠️ INVENTORY ALERT: Processing this edit will drop items into critical low stock territory.\n\nDo you wish to proceed?");
      if (!confirm) return; 
    }

    updateOfficialInvoice(formData);
  };
  ...
  return (
    <form action={clientAction} className="space-y-8 w-full min-w-0">
      ...
      {/* HEADER SECTION */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-white p-6 rounded-xl border border-neutral-200 shadow-sm w-full min-w-0">
        <div className="space-y-2.5 w-full min-w-0 md:col-span-1">
          <Label htmlFor="client_name" className="text-xs font-bold text-neutral-600 uppercase tracking-wider">Client Name</Label>
          <Input id="client_name" name="client_name" list="clients-list" defaultValue={invoice.client_name} className="h-11 w-full" required />
          <datalist id="clients-list">{customers?.map(c => <option key={c.id} value={c.name} />)}</datalist>
        </div>
        
        <div className="space-y-2.5 w-full min-w-0 md:col-span-1">
          <Label htmlFor="due_date" className="text-xs font-bold text-neutral-600 uppercase tracking-wider">Due Date</Label>
          <Input id="due_date" name="due_date" type="date" defaultValue={new Date(invoice.due_date).toISOString().split('T')[0]} className="h-11 w-full" required />
        </div>

        <div className="space-y-2.5 w-full min-w-0 md:col-span-1">
          <Label htmlFor="status" className="text-xs font-bold text-neutral-600 uppercase tracking-wider">Invoice Status</Label>
          <Select name="status" defaultValue={invoice.status} required>
            <SelectTrigger className="h-11 bg-white border-neutral-300 focus:ring-neutral-900"><SelectValue placeholder="Select status" /></SelectTrigger>
            <SelectContent>
          ...
          <div className="space-y-5 lg:space-y-2 w-full min-w-0">
            {items.map((item, index) => {
              ...
              if (dbItem && !isComposite) {
                // UI DELTA STATE CALCULATION FOR INLINE BADGES
                const originalItemRecord = safeInitialItems.find(i => i.item_id === item.item_id);
                const originalQty = originalItemRecord ? Number(originalItemRecord.quantity || 0) : 0;

                const currentStock = Number(dbItem.quantity_on_hand || 0);
                const orderQty = Number(item.quantity || 0);
                projectedStock = (currentStock + originalQty) - orderQty;
              ...
              return (
                ...
                >
                  ...
                  {/* 2. Description Input & INLINE WARNINGS */}
