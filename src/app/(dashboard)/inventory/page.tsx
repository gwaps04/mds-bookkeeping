...
export default async function InventoryPage(props: { searchParams: Promise<{ search?: string }> }) {
  ...
  // Fetch all 4 item types, ensuring archived (deleted) items remain hidden
  let query = supabase
    .from("items")
    .select("*")
  ...
  // Sort by updated_at so newly active items instantly jump to the top
  const { data: items } = await query.order("updated_at", { ascending: false });
        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          <Card className="shadow-sm border-neutral-200">
            ...
            <CardContent className="pt-4">
              <AddItemForm rawMaterials={items?.filter(i => i.type === 'RAW_MATERIAL') || []} />
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm border-neutral-200">
            ...
            <CardContent className="pt-4">
              {/* THE FIX: Interactive Client Component for instant form clearing! */}
