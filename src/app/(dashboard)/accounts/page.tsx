...
export default async function AccountsPage(props: { searchParams: Promise<{ search?: string, type?: string, view?: string, page?: string }> }) {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12 w-full min-w-0 overflow-x-hidden">
      ...
      <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4 md:p-5 shadow-sm w-full">
      ...
      {/* THE FIX 1: Add w-full and min-w-0 to the master grid to prevent Blowout */}
      <div className="grid gap-6 lg:gap-8 lg:grid-cols-3 items-start w-full min-w-0">
        
        {/* LEFT COLUMN: FORM */}
        <div className="lg:col-span-1 w-full min-w-0">
          {isLocked ? (
            <Card className="shadow-sm border-neutral-200 lg:sticky lg:top-8 bg-neutral-50/50 w-full min-w-0">
              <CardHeader>
            ...
            <Card className="shadow-sm border-neutral-200 lg:sticky lg:top-8 w-full min-w-0 bg-white">
            ...
            <Card className="shadow-sm border-neutral-200 lg:sticky lg:top-8 w-full min-w-0 bg-white">
              <CardHeader className="bg-neutral-50/50 border-b border-neutral-100 pb-4">
                <CardTitle className="text-lg font-bold text-neutral-900">Structural Controls</CardTitle>
        ...
        <div className="lg:col-span-2 space-y-4 w-full min-w-0">
            ...
            <CardContent className="p-4 md:p-5">
              <form method="GET" className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 sm:items-end w-full">
                ...
                <div className="flex-1 w-full min-w-[200px] space-y-1.5">
                  ...
                  {/* THE FIX 2: Added w-full to Input */}
                  <Input name="search" placeholder="Search by name..." defaultValue={params?.search} className="bg-neutral-50 border-neutral-200 focus-visible:ring-neutral-900 w-full" />
                </div>
                ...
                <div className="w-full sm:w-auto min-w-[140px] space-y-1.5">
                  ...
                  {/* THE FIX 3: Added w-full to SelectTrigger */}
                  <Select name="view" defaultValue={params?.view || "active"}>
                    <SelectTrigger className="bg-neutral-50 border-neutral-200 w-full"><SelectValue placeholder="Active" /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="w-full sm:w-auto min-w-[160px] space-y-1.5">
                  ...
                  <Select name="type" defaultValue={params?.type || "all"}>
                    <SelectTrigger className="bg-neutral-50 border-neutral-200 w-full"><SelectValue placeholder="All Types" /></SelectTrigger>
                    <SelectContent>
          ...
          {isArchiveView && (
            <div className="bg-amber-50 border border-amber-200 rounded-md p-3 flex items-center gap-2 text-amber-800 text-sm shadow-sm w-full">
                        ...
                        <tr key={acc.id} className={`hover:bg-neutral-50/60 transition-colors ${isArchiveView ? 'opacity-75 grayscale' : ''}`}>
                          ...
                          <td className="px-4 sm:px-6 py-4 text-center align-middle">
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-2">
                              ...
                              {isLocked ? (
                                <Button disabled variant="outline" size="sm" className="h-8 sm:h-9 px-3 text-xs bg-neutral-50 text-neutral-400 border-neutral-200 cursor-not-allowed w-full sm:w-auto font-bold shadow-sm">
                                ...
                                isArchiveView ? (
                                  <form action={async (formData) => {
                                    ...
                                    <SubmitButton 
                                      ...
                                      className="h-8 sm:h-9 px-3 text-xs bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm w-full sm:w-auto font-bold" 
                                    />
                                  </form>
                                ) : (
                                  <>
                                    <Link href={`/accounts/${acc.id}/edit`} className="w-full sm:w-auto">
                                      <Button variant="outline" size="sm" className="h-8 sm:h-9 px-3 text-xs bg-white text-blue-600 border-blue-200 hover:bg-blue-50 transition-colors w-full sm:w-auto font-bold shadow-sm">Edit</Button>
                                    </Link>
