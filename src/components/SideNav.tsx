...
export default function SideNav({ role, isTaxEnabled }: { role?: string, isTaxEnabled?: boolean }) {
  const pathname = usePathname();

  const isSuperAdmin = role === 'super_admin';
  const isBusinessOwner = role === 'business_owner';

  // Smart function that checks if the browser URL matches the link
  // (We use startsWith so if they are on /invoices/new, the /invoices tab stays highlighted!)
  const checkActive = (path: string) => pathname === path || (pathname.startsWith(path) && path !== '/dashboard');

  // A reusable link builder that automatically applies the active colors
    ...
    <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
      ...
      {isSuperAdmin ? (
        ...
        <>
          ...
          {/* --- CUSTOM COLOR HIGHLIGHTS FOR SPECIAL FEATURES --- */}
          {isTaxEnabled && (
